...
function windowBar(settings) {
  return `<header class="window-bar drag-region"><span class="window-name"><i class="window-mark"></i>本地保存</span><span class="window-actions no-drag"><button class="window-button" data-action="top" aria-label="${settings.alwaysOnTop ? '取消置顶' : '始终置顶'}" title="${settings.alwaysOnTop ? '取消置顶' : '始终置顶'}">${icons.pin}</button><button class="window-button" data-action="minimize" aria-label="最小化">${icons.minus}</button><button class="window-button close" data-action="hide" aria-label="关闭并隐藏">${icons.close}</button></span></header>`;
}
...
function entry(note, index, detailed = false) {
  ...
  const actionRow = !note.completedAt ? `<div class="entry-actions"><button class="text-action" data-action="add-child" data-id="${note.id}">+ 子 Note</button><button class="text-action delete" data-action="delete" data-id="${note.id}">删除</button></div>${addChild}` : `<div class="entry-actions"><button class="text-action delete" data-action="delete" data-id="${note.id}">删除</button></div>`;
  return `<article class="entry"><span class="entry-number">${String(index + 1).padStart(2, '0')}</span><div class="entry-main"><div class="entry-top">${checkbox(note)}${title}</div>${childRows ? `<div class="sublist">${childRows}</div>` : ''}${actionRow}${detailed ? `<time class="timestamp">创建于 ${formatDate(note.createdAt)}</time>` : ''}</div><span class="entry-progress">${note.children?.length ? `${note.children.filter((child) => child.completedAt).length} / ${note.children.length}` : '—'}</span></article>`;
}
...
async function render() {
  ...
  if (state.screen === 'widget') {
    app.className = 'app';
    app.innerHTML = `${windowBar(settings)}<section class="masthead"><span class="title">每日索引</span><span class="edition">今天 · 先记下</span></section><form class="composer" id="composer"><span class="plus">+</span><input id="note-input" aria-label="记录新的灵感" placeholder="写下一条，不必想清楚" autocomplete="off"><button class="save">收录</button></form><div class="notice">${escapeHtml(state.notice)}</div><div class="list-heading"><span>NO.</span><span>尚未完成的念头</span><span>进度</span></div><section class="entries">${notes.length ? notes.map((note, index) => entry(note, index)).join('') : '<div class="empty">这里还没有待办。<br>先把脑海里的那一句写下来。</div>'}</section><footer class="footer"><span class="footer-copy">主挂件只显示未完成的主 Note</span><button class="footer-button" data-action="all">全部记录 →</button></footer>`;
  } else {
    app.className = 'app';
